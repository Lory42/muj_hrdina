    ...
    loginForm.addEventListener('submit', async (event) => {
        ...
        try {
            ...
            if (error.code === 'auth/user-not-found' || error.code === 'auth/wrong-password' || error.code === 'auth/invalid-credential') {
                alert('Neplatné přihlašovací údaje. Zkontroluj e-mail a heslo.');
            } else {
...
if (registerForm) {
    registerForm.addEventListener('submit', async (event) => {
        ...
        try {
            ...
            alert('Registrace proběhla úspěšně. Na e-mail ti posílám ověřovací odkaz.');
            closeLoginModal();
        } catch (error) {
            console.error(error);
            if (error.code === 'auth/email-already-in-use') {
                alert('Tento e-mail již používáme.');
            } else if (error.code === 'auth/weak-password') {
                alert('Heslo je příliš slabé. Použij alespoň 6 znaků.');
            } else {
